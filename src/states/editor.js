...
const AUTO_SAVING_INTERVAL = 10e3; // Delay between each export to the local storage, in ms.
const ENTITY_MANDATORY_ATTRIBUTES = [
...
class EditorState extends Engine.State {
    constructor() {
        ...
        let bgColor = localStorage.getItem("settingBackgroundColor");
        ...
        this.setEventListener(new Engine.EventListener({
            ...
            onMouseUp: (button) => {
                ...
                if(button == 2 && lastMiddleDownLocation) {
                    let distanceSquared = Math.pow(app.mouse.x - lastMiddleDownLocation.x, 2) + Math.pow(app.mouse.y - lastMiddleDownLocation.y, 2);

                    if(distanceSquared < 9) {
                        if(currentTool == Tools.TILE_PLACER) { // Pick hovered tile id
                            let cell = editor.map.canvasPosToCell(app.mouse);
                            let id = currentLayer.tiles[cell.row][cell.col].id;
                            ...
                            for(let i = 0; i < currentLayer.entities.length; i++) {
                                let en = currentLayer.entities[i];
    ...
    onEnter(previousState) {
        ...
        $.getJSON("config/editor-config.json", (result) => {
            ...
            for(let typeName in this.config.entityTypes) {
                let type = this.config.entityTypes[typeName];
                for(let i = 0; i < ENTITY_MANDATORY_ATTRIBUTES.length; ++i) {
                    let attr = ENTITY_MANDATORY_ATTRIBUTES[i];
        ...
        this.map.onLayerRendered((layerIndex, ctx) => {
            let l = this.map.layers[layerIndex];
    ...
    getMapExportData(applyCompression) {
        ...
        for(let i = 0; i < this.map.layers.length; i++) {
            let mapLayer = this.map.layers[i];
            ...
            for(let r = 0; r < mapLayer.tiles.length; r++) {
                l.tiles[r] = [];
                for(let c = 0; c < mapLayer.tiles[r].length; c++)
    ...
    exportMap()  {
        let mapData = this.getMapExportData(true);
